...
class Plane(collections.namedtuple('Plane', ('point', 'normal'))):
	...
	def getPlanePN(cls, point, normal):
		if not isinstance(point, Math.Vector3):
			raise TypeError('point argument must be Vector3, not {}'.format(type(point).__name__))
		if not isinstance(normal, Math.Vector3):
			raise TypeError('normal argument must be Vector3, not {}'.format(type(normal).__name__))
	...
	def getPlanePVV(cls, point, vector0, vector1):
		...
		if not isinstance(vector0, Math.Vector3):
			raise TypeError('vector0 argument must be Vector3, not {}'.format(type(vector0).__name__))
		if not isinstance(vector1, Math.Vector3):
			raise TypeError('vector1 argument must be Vector3, not {}'.format(type(vector1).__name__))
		return cls(point, vector0 * vector1)

	@classmethod
	def getPlanePPP(cls, point0, point1, point2):
		if not isinstance(point0, Math.Vector3):
			raise TypeError('point0 argument must be Vector3, not {}'.format(type(point0).__name__))
		if not isinstance(point1, Math.Vector3):
			raise TypeError('point1 argument must be Vector3, not {}'.format(type(point1).__name__))
		if not isinstance(point2, Math.Vector3):
			raise TypeError('point2 argument must be Vector3, not {}'.format(type(point2).__name__))
	...
	def isVectorParallelPlane(self, vector):
		if not isinstance(vector, Math.Vector3):
			raise TypeError('vector argument must be Vector3, not {}'.format(type(vector).__name__))
	...
	def isPlaneParallel(self, plane):
		if not isinstance(plane, Plane):
			raise TypeError('plane argument must be Plane, not {}'.format(type(plane).__name__))
		return self._isPlaneParallel(plane)

	def intersectPlanes(self, plane0, plane1):
		if not isinstance(plane0, Plane):
			raise TypeError('plane0 argument must be Plane, not {}'.format(type(plane0).__name__))
		if not isinstance(plane1, Plane):
			raise TypeError('plane1 argument must be Plane, not {}'.format(type(plane1).__name__))
...
class BoundingSphere(collections.namedtuple('BoundingSphere', ('center', 'radius'))):
	...
	def __new__(cls, center, radius):
		if not isinstance(center, Math.Vector3):
			raise TypeError('center argument must be Vector3, not {}'.format(type(center).__name__))
		if not isinstance(radius, Math.Vector3):
			raise TypeError('radius argument must be Vector3, not {}'.format(type(radius).__name__))
